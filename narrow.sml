fun narrow(v,t,sigma,theta,gamma,cntr) = (case (v,t) of
	...
	| (VRecord(r1),TRecord(r2)) => 
		...
		let fun iterativeNarrow(l1,l2,s,t) = (case (l1,l2) of 
			...
			| ((labv,v1)::rest1,(labt,t1)::rest2) => 
				...
				then (case narrow(v1,t1,s,t,gamma,cntr) of 
	...
	| (VRecord(_),THole(TypeHole(ArithTypeVar(_)))) => Config(Stuck(cntr),sigma,theta)
	
	| (VRecord(r),THole(TypeHole(hole))) =>
		
		if Substitution.contains(TypeHole(hole),theta)
		then narrow(v,resolveChainTheta(THole(TypeHole(hole)),theta),sigma,theta,gamma,cntr)
	...
	| (VList(_),THole(TypeHole(ArithTypeVar(_)))) => Config(Stuck(cntr),sigma,theta)
	...
	| (Fun(_,t1,_),THole(TypeHole(TypeVar(a)))) =>
		...
		then narrow(v,resolveChainTheta(THole(TypeHole(TypeVar(a))),theta),sigma,theta,gamma,cntr)
			 ...
			 in (case unify([t,TFun(t1,freshType)],theta) of 
				(* unify to prevent type variable a being an element
				   ...
				   NONE 	   => Config(Stuck(cntr),sigma,theta)
				 | SOME theta1 => narrow(v,TFun(t1,freshType),sigma,theta1,gamma,cntr))
			 end
	...
	| (VHole(BinaryOpHole(ArithOper(oper),v1,v2)),t) => (case narrowExpr(ArithExpr(oper,Value(v1),Value(v2)),t,sigma,theta,gamma,cntr) of 
	...
	| (VHole(BinaryOpHole(BoolOper(oper),v1,v2)),t) => (case narrowExpr(BoolExpr(oper,Value(v1),Value(v2)),t,sigma,theta,gamma,cntr) of 
		(* Either, or both, of the arguments must be value holes *)
	...
	| (VHole(CaseHole(v1,patExprList)),t) => (case narrowExpr(Case(Value(v1),patExprList),t,sigma,theta,gamma,cntr) of
	...
	| (VHole(ConditionHole(v1,e1,e2)),t) => (case narrowExpr(Condition(Value(v1),e1,e2),t,sigma,theta,gamma,cntr) of
	...
	| (VHole(AppHole(v1,v2)),t) => (case narrowExpr(App(Value(v1),Value(v2)),t,sigma,theta,gamma,cntr) of
	...
	| (VHole(ConsHole(v1,v2)),t) => (case narrowExpr(Cons(Value(v1),Value(v2)),t,sigma,theta,gamma,cntr) of 
	...
	| (VHole(RecordHole(r)),t) =>
		...
		in narrowExpr(Record(valToERecord(r)),t,sigma,theta,gamma,cntr) end
	...
	| (VHole(ListHole(l)),t) =>
		...
		in narrowExpr(List(valToEList(l)),t,sigma,theta,gamma,cntr) end
		
	(* for anything that does not match the above clauses, can only return a stuck expression *)
	| _  => Config(Stuck(cntr),sigma,theta))
	   
and narrowExpr(e,t,sigma,theta,gamma,cntr) = (case (e,t) of 

	  (Value(v),t) => narrow(v,t,sigma,theta,gamma,cntr)
	...
	| (ArithExpr(DIVIDE,e1,e2),THole(hole)) => (case unify([t,Real],theta) of 
		  ...
		  NONE 		  => Config(Stuck(cntr),sigma,theta)
		| SOME theta1 => narrowExpr(e,Real,sigma,theta1,gamma,cntr))
	
	(* cannot narrow e1/e2 to anything else
	   needed so doesn't match arith expr clause below using wildcard *)
	| (ArithExpr(DIVIDE,_,_),_) => Config(Stuck(cntr),sigma,theta)
	...
	| (ArithExpr(oper,e1,e2),t) => (case t of 
		  ...
		  Bool 		 => Config(Stuck(cntr),sigma,theta)
		| TRecord(_) => Config(Stuck(cntr),sigma,theta)
		| TList(_)   => Config(Stuck(cntr),sigma,theta)
		| TFun(_,_)  => Config(Stuck(cntr),sigma,theta)
		
		| THole(TypeHole(TypeVar(tyvar))) => 
			if Substitution.contains(TypeHole(TypeVar(tyvar)),theta)
			then narrowExpr(e,resolveChainTheta(t,theta),sigma,theta,gamma,cntr)
			else let val freshArith = generateFreshTypeVar(ARITH_TYPE_VAR,theta)
				 in narrowExpr(e,freshArith,sigma,Substitution.union(theta,TypeHole(TypeVar(tyvar)),freshArith),gamma,cntr) end
		
		| THole(TypeHole(EqualityTypeVar(tyvar))) =>  
			if Substitution.contains(TypeHole(EqualityTypeVar(tyvar)),theta)
			then narrowExpr(e,resolveChainTheta(t,theta),sigma,theta,gamma,cntr)
			else narrowExpr(e,Int,sigma,Substitution.union(theta,TypeHole(EqualityTypeVar(tyvar)),Int),gamma,cntr)
	...
	| (BoolExpr(EQ,e1,e2),THole(hole)) => (case unify([t,Bool],theta) of 
		  ...
		  NONE 		  => Config(Stuck(cntr),sigma,theta)
		| SOME theta1 => narrowExpr(e,Bool,sigma,theta1,gamma,cntr))
	
	(* cannot narrow e1=e2 to anything else 
	   needed so doesn't match arith expr clause below using wildcard *)
	| (BoolExpr(EQ,_,_),_) => Config(Stuck(cntr),sigma,theta)
	...
	| (Record(r1),TRecord(r2)) => 
		...
		let fun iterativeNarrowExpr(l1,l2,s,t) = (case (l1,l2) of 
			...
			| ((labe,e1)::rest1,(labt,type1)::rest2) =>
				...
				then (case narrowExpr(e1,type1,s,t,gamma,cntr) of 
	...
	| (Record(_),THole(TypeHole(ArithTypeVar(_)))) => Config(Stuck(cntr),sigma,theta)
	
	(* For general type variable or equality type variable *)
	| (Record(r1),THole(TypeHole(hole))) =>
	
		if Substitution.contains(TypeHole(hole),theta)
		then narrowExpr(e,resolveChainTheta(THole(TypeHole(hole)),theta),sigma,theta,gamma,cntr)
			 ...
			 in narrowExpr(e,genType,sigma,theta1,gamma,cntr) end
	
	| (List(eList),TList(listType)) =>
	
		let fun iterativeNarrowExpr(l,s,t) = (case l of
		
			  [] 	   => Config(Expression(List([])),s,t)
			| e1::rest => (case narrowExpr(e1,listType,s,t,gamma,cntr) of 
	...
	| (List(_),THole(TypeHole(ArithTypeVar(_)))) => Config(Stuck(cntr),sigma,theta)
	...
	| (Case(e1,patExprList),t) => 
		...
		let (* To make overall case expression capture avoiding, we do so 
			...
			(* To get type we need to narrow e1 to, look at the form of patterns
			   - For a single pattern case, for all cases except records and lists, 
				 ...
				 - For :: need to generate a list type, of the type generated
				   ...
				   (in case it is a list of records/lists)
			...
			in (case narrowExpr(e1,e1NarrowType(getPatList(patExprList)),sigma,theta,gamma,cntr) of
	...
	| (Let(x,tX,e1,e2),t) => 
		...
		then let val counter = getCounterAndUpdate()
			 in narrowExpr(Let(alphaVariable(x,counter,[x]),tX,e1,alphaExpr(e2,counter,[x])),t,sigma,theta,gamma,cntr) end
	...
	| (LetRec(x,TFun(t1,t2),Fun(y,t3,e1),e2),t) =>
		...
		then narrowExpr(alphaExpr(e,getCounterAndUpdate(),[x]),t,sigma,theta,gamma,cntr)
